...
void CalculateBATVoltage() {
  ...
  unsigned int TmpVbatArray[VoltageAverageValue];

  

  if ((EnDischarge == true) && (DischargeState == 1)) {
    digitalWrite(DISCHARGE_ON, LOW);
    if(DischargePercent < 5){
      delay(20);
    }else{
  ...
  for (int i = 0; i <= (VoltageAverageValue-1); i++) {
  ...
  for (int i = 0; i <= (VoltageAverageValue-1); i++){
    TmpVbatAvg += TmpVbatArray[i];
  }

  TmpVbatAvg = TmpVbatAvg / VoltageAverageValue;
  ...
  DischargeDelta = DischargeVoltage - AvgCellVoltage;
